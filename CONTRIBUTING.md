...
In addition to maintaining this module I also maintain the [redis_protocol](https://github.com/aembke/redis-protocol.rs) crate. While adding RESP3 support to `redis_protocol` I encountered a few issues that may be relevant to any contributors working on this library.

In the [README](./README.md) I mention that there are some aspects of the [RESP3 spec](https://github.com/antirez/RESP3/blob/master/spec.md) that are ambiguous. In the interest of explaining some design decisions and preventing future bugs I'll list them here:

* Despite an entire section on value attributes I cannot find a single example of them being used in the real world. From what I can tell they are not used at all in any existing Redis commands, however this may be incorrect since I only tested a few categories. Either way, the lack of clear documentation on this lead to the decision to hide attributes entirely in this library. Until we know whether attributes need to be represented recursively on any `RedisValue` enum, or whether they only need to appear with aggregate types, etc, I would prefer to just hide them. If they do eventually need to be surfaced to callers that will be a very invasive PR. 
* The `HELLO` command fails to mention that it requires a CRLF terminating byte sequence, which I found out via trial and error. Additionally, `HELLO` strikes me as very strange since it is the one command that does not use separate bulk/blob strings (with their own inner CRLF delimiters) for words. While the current implementation seems to work for the purposes of switching protocols, I am hesitant to use `HELLO` for auth and client-setname use cases since I don't know what else might be missing in the spec here. To avoid problems here this library will continue to use separate `AUTH` and `CLIENT SETNAME` commands despite the fact that `HELLO` can do both in one command. Additionally, the `HELLO` documentation in the spec already seems out of date, since `redis-cli` will try autocompleting a `SETNAME` suffix, yet the spec makes no mention of this, and I have no idea which version of Redis added this.
* The aggregate Map and Set types are somewhat ambiguous on the valid key types. I can understand why, since clients may use different Map implementations, but it doesn't make sense to use another aggregate type as a key in a map in any situation. However, there's nothing in the protocol that seems to prevent this. In fact, in the `Hash` implementation for the RESP3 `Frame` type in `redis_protocol` there's a special check that will panic if the server does this for some reason. It would have been nice if the spec were more clear on valid key types (such as only strings, or something like `FromStr`, or even just saying "no aggregate types").  
* The Set implementation can be strange for nested aggregate types. To my knowledge there aren't any commands that do this currently, but in reading the protocol it certainly seems possible to send a Set that contains other aggregate types. This presents a problem when implementing `Ord` or `Hash` on a `Frame`. How should one order or hash nested aggregate types? Further, iteration order over `HashMap` and `HashSet` is undefined, so trying to do some nested hashing logic will not work either. As it stands today there are only two cases where nested aggregate types are supported in this library - arbitrarily nested arrays and arrays within sets. Trying to use a map within a set, or a set within a set, etc, will panic. There are probably clever ways to make this work (likely using BTrees, or something that enforces ordering), but I'd prefer to see a real use case for it first. This is further complicated by the fact that the `FromRedis` trait supports converting values to `HashMap` and `HashSet`, and I really don't want to remove that. For example, the spec says the following:
  ```
  Maps can have any other type as field and value, however Redis will use only a subset of the available possibilities. For instance it is very unlikely that Redis commands would return an Array as a key, however Lua scripts and modules will likely be able to do so.
  ```
  While that might work for arrays, if you swap "Array" and "Map" (which is allowed according to earlier in the spec) it no longer makes sense. I can see how it might work for BTrees, but that would then contradict the earlier statements in the spec about not forcing clients into certain implementations. If you think this is incorrect please tell me - I would really like to remove some `panic` calls in `redis_protocol`. Who knows, maybe we can just `#[derive(Hash)]` on a `HashMap` or `HashSet` and it somehow works deterministically... 

The general theme with RESP3 that I noticed is that there are some hairy implementation considerations with certain types, particularly the map and set aggregate types (since we can no longer rely on ordering constraints like we do with arrays), but Redis gets around that by simply avoiding these issues in practice. That being said, since we're using Rust here we need to prove to the compiler that the implementation makes sense in all situations, and in some cases it just doesn't without further constraints that are not expressed in the spec. To make this work I had to break my cardinal rule of "never panic" in the protocol implementation, which was not encouraging, and lead to my writing this section in the docs.
