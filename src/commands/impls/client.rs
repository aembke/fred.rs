...
pub async fn echo<C: ClientLike>(client: &C, message: &str) -> Result<RedisValue, RedisError> {
  let frame = utils::request_response(client, move || Ok((RedisCommandKind::Echo, vec![message.into()]))).await?;
