...
#[cfg(feature = "replicas")]
pub fn route_replica(router: &mut Router, command: &RedisCommand) -> Result<(Server, Server), RedisError> {
...
pub async fn send_asking_with_policy(
  ...
  macro_rules! next_sleep {
    ($err:expr) => {{
      let delay = match next_reconnection_delay(inner) {
        ...
        Err(_) => {
          return Err(
            $err.unwrap_or_else(|| RedisError::new(RedisErrorKind::Routing, "Unable to route command or reconnect.")),
          );
        },
      };
  ...
  loop {
    ...
    if attempts_remaining == 0 {
      return Err(RedisError::new(RedisErrorKind::Routing, "Max attempts reached."));
    }
